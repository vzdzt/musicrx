...
const API_BASE = window.location.hostname === 'localhost'
  ? 'http://localhost:3000'
  : 'https://musicrx.app'; // Production VPS backend URL
...
async function loadUndergroundArtists() {
    try {
        ...
        if (undergroundArtists.length > 0) {
            let html = '';
            // Show top 6 underground artists (already sorted from API)
        ...
        if (tweets.length > 0) {
            ...
            tweets.forEach((tweet) => {
                ...
                html += `<a href="${tweet.url}" target="_blank" class="scroll-item" style="text-decoration: none;">
                    ...
                    <div class="news-content-card">
                        <p class="date">@${source}</p>
                        ${tweet.imageUrl ? `<img src="${tweet.imageUrl}" alt="${tweet.title}">` : '<div style="width: 100%; height: 120px; background: var(--card-background); border-radius: 4px; display: flex; align-items: center; justify-content: center;"><i class="fab fa-twitter" style="font-size: 2rem; opacity: 0.5;"></i></div>'}
                        <p style="font-size: 0.8rem; color: var(--secondary); margin-top: 0.5rem;">${publishedDate}</p>
                    </div>
                </a>`;
            });
        ...
        if (aotyContenders.length > 0) {
            ...
            topContenders.forEach((album) => {
                ...
                html += `<a href="all-2025-albums.html" class="scroll-item" style="text-decoration: none;">
                    ...
                    <div class="news-content-card">
                        <p class="date">${artist}</p>
                        ${album.imageUrl ? `<img src="${album.imageUrl}" alt="${album.title}">` : '<div style="width: 100%; height: 120px; background: var(--card-background); border-radius: 4px; display: flex; align-items: center; justify-content: center;"><i class="fas fa-music" style="font-size: 2rem; opacity: 0.5;"></i></div>'}
